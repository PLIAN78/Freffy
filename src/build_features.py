...
def main():
    ...
    for _, lab in labels.iterrows():
        ...
        if pd.isna(lab.get("first_action_frame")) or pd.isna(lab.get("y_first_action_is_R")):
            print(f"Skipping (missing label): {clip}")
            continue

        y = int(lab["y_first_action_is_R"])
        ...
        distance_closing = float(dists[0] - dists[-1]) if len(dists) >= 2 else 0.0

        rows.append({
            "clip": clip,
            "y_first_action_is_R": y,
            "L_speed_mean": L_mean,
            "L_speed_max": L_max,
            "R_speed_mean": R_mean,
            "R_speed_max": R_max,
